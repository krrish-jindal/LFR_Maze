...
const int irSensor5Pin = A4;    // Right most sensor
...
void loop()
{
  int irSensor1Val = analogRead(irSensor1Pin);
  int irSensor2Val = analogRead(irSensor2Pin);
  int irSensor3Val = analogRead(irSensor3Pin);
  int irSensor4Val = analogRead(irSensor4Pin);
  int irSensor5Val = analogRead(irSensor5Pin);
...
//}
  ...
  if (irSensor3Val<blackThreshold) {
    // Robot is off course
   forward();
   
    Serial.println(irSensor3Val);
  } 
  else if (irSensor2Val<blackThreshold) {
    // Robot is to the right of the line
    left();
    Serial.println(irSensor3Val);
  } 
  else if (irSensor4Val<blackThreshold) {
    // Robot is to the right of the line
   right();

  }
  else if(irSensor5Val<blackThreshold){
    sharpLeftTurn();
    } 
  else if(irSensor1Val<blackThreshold){
    sharpRightTurn();
    }
  else{
    stop1();
